...
//Intel
// Adds search action to dead units
...
{[_x, 0, ["ACE_MainActions"], _action, false] call ace_interact_menu_fnc_addActionToClass;} forEach (btc_type_units + btc_type_divers);

// Adds interrogate action to captured units
...
{ //Actions attachted to AI
    ...
    [_x, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;
    _action = ["Give_food", localize "STR_BTC_HAM_ACTION_GIVEFOOD", "\z\ace\addons\field_rations\ui\item_waterbottle_full_co.paa", {[player, _target] call btc_int_fnc_foodGive}, {alive _target && {"ACE_WaterBottle" in items player}}] call ace_interact_menu_fnc_createAction;
    [_x, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;
