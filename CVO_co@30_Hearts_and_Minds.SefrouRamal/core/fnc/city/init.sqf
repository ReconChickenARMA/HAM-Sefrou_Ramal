...
for "_id" from 0 to (count _locations - 1) do {
    ...
    if (_type in _citiesType) then {
        ...
        { 
            if ((getMarkerPos _x) inArea [_position, 500, 500, 0, false]) exitWith { _isSafeZone = true };
        } forEach allMapMarkers select {"safezone" in _x || {"btc_base" in _x}};;
        if (_isSafeZone) exitWith {};
