...
//_sound = playSound3D [...] once 2.14 comes out, this sound can be stopped

if (_duration < 10) then {
	playSound3D [getMissionPath "cvo\IED\sound\burning_fuse_7.ogg", _particleSource,false,(getPos _particleSource),3,1.4,200,0,true];
	} else {
	playSound3D [getMissionPath "cvo\IED\sound\burning_fuse_23.ogg", _particleSource,false,(getPos _particleSource),3,1.4,200,0,true];
};
...
while {time < _startTime + _duration} do {
	...
	switch (_intensity) do
	{
		case 0: { _multi =  1; };
		case 1: { _multi = -1; };
		default { if ((random 1) > 0.7) then { _multi = -1 * _multi; } };
	};
